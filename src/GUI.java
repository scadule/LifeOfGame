...
public class GUI extends JFrame implements ActionListener {
    ...
    public void initGUI() {
        ...
        map = new Map(maxWidth,maxLength);
        ...
        for (int i = 0; i < maxWidth; i++) {
            for (int j = 0; j < maxLength; j++) {
                ...
                nGrid[i][j].setBackground(Color.CYAN); //初始时所有细胞均为死亡状态
        ...
        jbNowGeneration = new JButton(""+map.getNowGeneration());//Button不能直接添加int，故采用此方式
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == ok) { //确定
            setMaxLength(lengthList.getSelectedIndex() + 3);//设置的初始最小值为3，故加上3，下同
            setMaxWidth(widthList.getSelectedIndex() + 3);
            initGUI();

            map = new Map(getMaxWidth(), getMaxLength());
            ...
            isRunning = false;
            thread = null;
        } else if (e.getSource() == start) { //开始繁衍
            isRunning = true;
            thread = new Thread(new Runnable() {
                public void run() {
                    while (isRunning) {
                        ...
                        if (isDead) {
                            JOptionPane.showMessageDialog(null, "所有细胞均已死亡");
                            isRunning = false;
                            thread = null;
                        }
                    }
            ...
            for (int i = 0; i < maxWidth; i++) {
                for (int j = 0; j < maxLength; j++) {
                    if (e.getSource() == nGrid[i][j]) {
                        ...
                        if (isSelected[i][j]) {
                            nGrid[i][j].setBackground(Color.MAGENTA);
                            grid[i + 1][j + 1] .setStatus(true);
                        } else {
                            nGrid[i][j].setBackground(Color.CYAN);
                            grid[i + 1][j + 1].setStatus(false);
                        }
                        break;
                    }
                }
    ...
    public void showCell(){
        ...
        for (int i = 0; i < maxWidth; i++) {
            for (int j = 0; j < maxLength; j++) {
                if (grid[i + 1][j + 1].getStatus()) {
                    nGrid[i][j].setBackground(Color.MAGENTA); //活显示紫色
                } else {
                    nGrid[i][j].setBackground(Color.CYAN); //死则显示天蓝色
                }
            }
        }
    }
